...
Use the `getCurrentUser()` function from the [Session cookies in Next.js](/lucia-next-docs-starlight/sessions/cookies/nextjs) page to get the current user and session.

<Aside title="Note from author">I have no clue what pilcrow was referring to here, as  and the function itself doesn't exist anywhere in the docs except the tutorials.</Aside>
