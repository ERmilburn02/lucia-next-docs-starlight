---
title: "Lucia Next"
description: An open source project to provide resources on implementing authentication with JavaScript and TypeScript.
template: splash

next:
    link: /lucia-next-docs-starlight/sessions/overview/
    label: Sessions overview
hero:
  tagline: An open source project to provide resources on implementing authentication with JavaScript and TypeScript.
  image:
    file: ../../assets/lucia-logo-square-transparent.webp
    alt: Lucia logo
  actions:
    - text: Sessions overview
      link: /lucia-next-docs-starlight/sessions/overview/
      icon: right-arrow
    - text: GitHub OAuth Tutorial
      link: /lucia-next-docs-starlight/tutorials/github-oauth/
      icon: right-arrow
      variant: minimal
---
...
<Aside title="Note from author">
Hi, it's me, ERmilburn02, the one that rebuilt these docs using Starlight. When you see blocks with this title, it's a note directly from me, that didn't exist in the original Lucia Next Docs. It's usually reserved only for parts that are broken or missing (which you'll see a lot in Next.js areas).

It should also be noted that these docs aren't automatically updated, but I'll try to keep them up to date as much as possible. The site is currently in sync with [`lucia-auth/next@33a63fd`](https://github.com/lucia-auth/next/commit/33a63fd4da571ee56b0327244a494f91537ddd10).
</Aside>
